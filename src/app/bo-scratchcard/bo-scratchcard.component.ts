...
@Component({
  selector: 'app-bo-scratchcard',
  templateUrl: './bo-scratchcard.component.html',
  styleUrls: ['./bo-scratchcard.component.scss'],
})

export class BoScratchcardComponent {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<BoScratchcardComponent>,
    ...
    this.location_parentcustomerid = 0;
    ...
    /*this._orderservice.addOnlineScratchOrder(JSON.stringify(this.order))
      .subscribe(
        response => {
          ...
          FileSaver.saveAs(file, 'Scratchcard' + '-' + moment().format('D-M-YYYY H_m') + '.xlsx');
          this.buttondisabled = false;
        ...
        response => {
          ...
          if ((response != undefined)) {
            if (response.status == 200) {
              let file = new Blob([response.body], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
              FileSaver.saveAs(file, 'Kraskaarten' + '-' + moment().format('D-M-YYYY H_m') + '.xlsx');
              this.buttondisabled = false;
            }
          } else {

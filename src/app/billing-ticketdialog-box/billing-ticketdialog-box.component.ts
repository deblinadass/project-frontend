...
@Component({
  selector: 'app-billing-ticketdialog-box',
  templateUrl: './billing-ticketdialog-box.component.html',
  styleUrls: ['./billing-ticketdialog-box.component.scss']
})

export class BillingTicketdialogBoxComponent {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<BillingTicketdialogBoxComponent>,
  ...
  ngOnInit() {
    ...
    if (this.local_data.action == 'Add') {
      ...
      this.ticketcreateform = this.formBuilder.group({
        ...
        billingaccountownerplace: ['', Validators.required],
        billingaccountnumber: ['', Validators.required],
        billingaccountownerfirstname: ['', Validators.required],
        billingaccountownermiddlename: [''],
        billingaccountownerlastname: ['', Validators.required],
  ...
  onSubmit(form: NgForm) {
    ...
    form.value.issuestate = '200';
    form.value.tickettype = 2;
    ...
    this.TicketFormUpdatedValue.push(
      {
        ...
        billingname: this.ticketcreateform.get('billingname').value,
        billingaccountownerplace: this.ticketcreateform.get('billingaccountownerplace').value,
        billingaccountnumber: this.ticketcreateform.get('billingaccountnumber').value,
        billingaccountownerfirstname: this.ticketcreateform.get('billingaccountownerfirstname').value,
        billingaccountownermiddlename: this.ticketcreateform.get('billingaccountownermiddlename').value,
        billingaccountownerlastname: this.ticketcreateform.get('billingaccountownerlastname').value,
  ...
  populateformdropdown() {
    ...
    this.ticketservice.getcommunicationgroupticket('2').subscribe(data => {
    ...
    this.ticketservice.getIssueProperty('issuetype',2).subscribe(data => {
      this.issuetypeList = data;
      for (let isuue of this.issuetypeList) {
        if (Number(isuue.id) === 20 || Number(isuue.id) === 60) {
          this.selectedIssueType = isuue.id;
          this.billingissuetype.push({
            id: isuue.id,
            name: isuue.name
          });
        }
    ...
    this.ticketservice.getIssueProperty('installprovider',2).subscribe(data => {
      this.installproviderList = data;

      for (let ip of this.installproviderList) {
        if (ip.id === "110") {
          this.selectedInstallProvider = ip.id;
    ...
    this.ticketservice.getIssueProperty('urgency',2).subscribe(data => {
      this.urgencyList = data;
    }, (err) => {
      this.dialogRef.close({ event: 'Cancel' });
    });

    this.ticketservice.getIssueProperty('channel',2).subscribe(data => {
    ...
    this.ticketservice.getIssueProperty('issuestatus',2).subscribe(data => {

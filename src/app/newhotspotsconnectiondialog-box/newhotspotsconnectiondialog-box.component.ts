...
@Component({
  selector: 'app-newhotspotsconnectiondialog-box',
  templateUrl: './newhotspotsconnectiondialog-box.component.html',
  styleUrls: ['./newhotspotsconnectiondialog-box.component.scss'],
})

export class NewhotspotsconnectiondialogBoxComponent {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<NewhotspotsconnectiondialogBoxComponent>,
  ...
  ngOnInit() {
    ...
    if (this.local_data.action == 'Add') {
      ...
      this.selectConnectionUpdate(this.local_data.BOUWSOConnection);
      ...
      this.local_data.BOUWSOrderContractor = Number(this.local_data.BOUWSOrderContractor);
      this.local_data.BOUWSOLinequantity = (this.local_data.BOUWSOLinequantity && this.local_data.BOUWSOLinequantity!='') ? Number(this.local_data.BOUWSOLinequantity) : '';
      ...
      this.hotspotscreateform = this.formBuilder.group({
        ...
        BOUWOrderWishdate: [this.local_data.BOUWOrderWishdateEdit],
        BOUWOrderContractor: [Number(this.local_data.BOUWOrderContractor)],
        BOUWOrderPlanneddate: [this.local_data.BOUWOrderPlanneddateEdit],
        BOUWOrderCabling: [this.local_data.BOUWOrderCabling],
        description: [this.local_data.BOUWSODescription],
        BOUWSOStartpacket: [this.local_data.BOUWSOStartpacket],
        BOUWSOConnection: [this.local_data.BOUWSOConnection, Validators.required],
        ...
        BOUWSODescription: [this.local_data.BOUWSODescription], // Glas/DSL description
        ...
        BOUWSOrderContractor: [Number(this.local_data.BOUWSOrderContractor), Validators.required],
        BOUWSOrderWishdate: [this.local_data.BOUWSOrderWishdate, Validators.required],
        BOUWOrderCoverage: [''],
        BOUWOrderSelectedType: [this.local_data.BOUWOrderSelectedType],
        BOUWOrderTerminationReason: [this.local_data.BOUWOrderTerminationReason]
        /****** New Attr ******/
    ...
    this.orderbase = [
      {
        ...
        orderstatus: '1',
        orderstate: '1',
        remarks: (this.hotspotscreateform.get('BOUWSODescription').value == '') ? '' : this.hotspotscreateform.get('BOUWSODescription').value.toString().trim(),
        creationdate: this.jstoday,
        updationdate: null,
        ponumber: '',
        ordertype: '1',
        locationid: this.locationid,
        productcatalogueid: 3, //Bouw Type
        productcataloguegroup: 3, //Bouw Type
        ...
        isstartpaketactive: '0',
        startpaketcatalogue: 0,
        stratpaketname: '0',
        isconnection: this.hotspotscreateform.get('BOUWSOConnection').value,
        bouwtype: (this.hotspotscreateform.get('BOUWSOConnection').value == 1) ? 4 : 3, //Glas/DSL
		ordermcid: 0,
      }
    ];
  ...
  onUpdate(form: NgForm) {
    ...
    this.orderbase = [
      {
        ...
        orderstatus: this.local_data.orderstatus,
        orderstate: this.hotspotscreateform.get('orderstate').value,
        remarks: (this.hotspotscreateform.get('BOUWSODescription').value == '') ? '' : this.hotspotscreateform.get('BOUWSODescription').value.toString().trim(),
        creationdate: this.local_data.creationdateEdit,
        updationdate: this.jstoday,
        ponumber: '',
        ordertype: this.local_data.ordertype,
        locationid: this.locationid,
        ...
        stratpaketname: '0',
        isconnection: this.hotspotscreateform.get('BOUWSOConnection').value,
        bouwtype: (this.hotspotscreateform.get('BOUWSOConnection').value == 1) ? 4 : 3,
		//doubt
        ordermcid: 10,
      }
    ];
    ...
    var ConnTypeID = (this.hotspotscreateform.get('BOUWSOConnection').value == '1') ? '4' : '3';
    ...
    var BOUWOrderContractorName = this.hotspotscreateform.get('BOUWSOrderContractor').value ? this.communicationgroupList.find(r =>
      r.communicationvalue == this.hotspotscreateform.get('BOUWSOrderContractor').value
    ).communicationname : '';
    ...
    this.orderFormUpdatedValue.push({
      ...
      BOUWSOrderWishdate: this.hotspotscreateform.get('BOUWSOrderWishdate').value ? formatDate(this.hotspotscreateform.get('BOUWSOrderWishdate').value, 'dd-MM-yyyy', 'en-US') : '',
      BOUWSOrderWishdateEdit: this.hotspotscreateform.get('BOUWSOrderWishdate').value ? formatDate(this.hotspotscreateform.get('BOUWSOrderWishdate').value, 'yyyy-MM-dd', 'en-US') : '',

      BOUWSOrderContractor: this.hotspotscreateform.get('BOUWSOrderContractor').value,
      ...
      BOUWOrderPlanneddateEdit: this.hotspotscreateform.get('BOUWOrderPlanneddate').value ? formatDate(this.hotspotscreateform.get('BOUWOrderPlanneddate').value, 'yyyy-MM-dd', 'en-US') : '',
      ...
      BOUWSODescription: this.hotspotscreateform.get('BOUWSODescription').value,
  ...
  getOrderStateList() {
    this._orderservice.getorderdetails(this.local_data.id).subscribe(data1 => {
      this.orderdetails = data1;  
      this._orderservice.getBouwNextStateProperty(this.orderdetails.orderstate).subscribe(data => {
        ...
        this.selectedState = Number(this.orderdetails.orderstate);
      }, (err) => {
        this.dialogRef.close({ event: 'Cancel' });
        this.toasterservice.showError(err);
      });
  ...
  selectConnectionUpdate(checkValue) {
    if (checkValue == '1') {
      this.isConn = checkValue;
    } else {
      this.isConn = checkValue;
